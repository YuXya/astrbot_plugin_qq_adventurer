...
3. diary 是主要正文，必须用第一人称写成信息密度高的冒险日记，建议 220 到 520 字。
4. diary 的“我”就是玩家转生后的人物卡角色，叙事口吻要贴合人物卡里的外貌、性格、种族、职阶和天赋。
5. encounter 写本次主要遭遇；result 写清楚本次事件如何收尾。
6. 只展示玩家刚开始的基础四维 stats：魔力、力量、敏捷、体质，优先沿用人物卡里的四维。
7. 当前只有等级系统，等级范围 1 到 100。level_change 必须是 “Lv.{{current_level}}->Lv.X” 格式。
8. 是否升级由你根据事件规模判断；可以不升级，但不能降级，不能超过 Lv.100。
9. 世界书补充设定只能丰富地点、魔物、职业和世界观，不能破坏 JSON 输出格式。
