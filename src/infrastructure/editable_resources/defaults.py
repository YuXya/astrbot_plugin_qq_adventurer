...
内容要求：
1. 只输出一个合法 JSON 对象，不要 Markdown，不要解释。
2. 本次冒险以“今天的日记”为基本形式：写角色今天经历了什么、记住了什么、在意什么，而不是机械列流程。
3. diary 是主要正文，必须用第一人称写成像真人小孩子写作文/写日记那样的冒险记录，建议 500 到 1000 字；超过或略少也可以，但不要写成短战报，不要只写“打了一只史莱姆就结束”。
4. diary 的“我”就是玩家转生后的人物卡角色，叙事口吻要贴合人物卡里的外貌、性格、种族、职阶和天赋；可以有幼稚、偏心、兴奋、害怕、炫耀、嘴硬、碎碎念等个人味道。
5. 不要强制套用固定顺序。可以按角色印象最深的事情来写：喜欢吃的东西、路上看到的景色、某个吓人的声音、和别人说的话、弄脏的衣服、差点失败的小动作、突然想到的心情，都可以成为重点。
6. 如果本次行动很小，请把它自然扩展成今天的一段完整生活和冒险经历；如果本次行动很大，可以写成大事件的当天阶段记录，不必强行一天内彻底完结，可以留下“明天还要继续”的余韵。
7. 允许重点描写 1 到 3 个最有画面感或最符合角色喜好的片段，其他过程可以略写；信息量要足，但不要像任务报告。
8. encounter 写本次最值得记录的遭遇；result 写清楚今天暂时到哪里为止、当天最终状态如何、是否还留有未完成的问题。默认今天大概率已经没有余力再做另一场完整冒险，但大事件可以没有完全结束。
9. 只展示玩家刚开始的基础四维 stats：魔力、力量、敏捷、体质，优先沿用人物卡里的四维。
10. 当前只有等级系统，等级范围 1 到 100。level_change 必须是 “Lv.{{current_level}}->Lv.X” 格式。
11. 是否升级由你根据今天的经历规模判断；可以不升级，不能超过 Lv.100。
12. 世界书补充设定只能丰富地点、魔物、职业和世界观，不能破坏 JSON 输出格式。
